...
solve equations =
    array (bounds equations)
          [(i,x) | i <- indices equations, let Right x = solution!i]
    where solution = construct (array (lb,ub') gl) t
          (lb,ub) = bounds equations
          (ub',gl,t) = foldl phi (ub,[],emptyFM) (assocs equations)
          phi (n,gl,t) (lhs, rhs@(Set sys v)) =
              (ub+off, gl'++gl, addListToFM t t')
              where g = sysGraph sys
                    (lb,ub) = bounds g
                    off = n + 1 - lb
                    gl' = [(lhs, map f (g!v))] ++
                          [(x+off, map f children) | (x,children) <- assocs g]
                          where f x = case lookupFM (sysTagging sys) x of
                                      Just (Right v) -> v
                                      _ -> x+off
                    t' = [ p
                           | (v,x) <- fmToList (sysTagging sys)
                           , p <- (case x of
                                   Left y  -> [(v+off, y)]
                                   Right _ -> []) -- XXX
                         ]
...
- 生き残らない点
  - 右辺のルート               => 左辺の点に置き換わる
  - 変数が割り当てられている点 => 変数の番号に置き換わる


gl' = [(m!x, [m!ch | ch <- g!x]) | x<-xs]
m = array (bounds g) m'
(ub',xs,m',t') = foldl phi (ub, [], [], t) (indices g)
    where phi (ub, xs, m, t) x
              | v==x      = (ub, xs, (x,lhs):m, t)
              | otherwise =
                  case lookupFM (sysTagging sys) x of
                  Just (Right n) -> (ub, xs, (x,n):m, t)
                  Just (Left u)  -> (ub+1, x:xs, (x,ub+1):m,
                                     t `addToFM` (ub+1,u))
