...
mkSet xs = case construct (array (0,n) ((n,children):gl)) t ! n of
           Right set -> set
    where (n,children,gl,t) = foldl phi (0,[],[],emptyFM) xs
          phi (n,s,gl,t) (Left u) = (n+1, n:s, (n,[]):gl, addToFM t n u)
          phi (n,s,gl,t) (Right (Set sys v)) =
              ( ub+off+1
              , (v+off) : s
              , gl'++gl
...
solve equations =
    ...
    where solution = construct (array (lb,ub') gl) t
          ...
          (ub',gl,t) = foldr phi (ub,[],emptyFM) (assocs equations)
          phi (lhs, rhs@(Set sys v)) (n,gl,t) = (ub+off, gl', t')
              where g = sysGraph sys
                    (lb,ub) = bounds g
                    off = (n+1) - lb
                    h x = case lookupFM (sysTagging sys) x of
                          Just (Right v) -> v
                          _ -> x+off
                    gl' = [(lhs, [h ch | ch <- g!v])] ++
                          [(v'+off, [h ch | ch <- children])
                           | (v',children) <- assocs g]
                          ++ gl
                    t' = t `plusFM`
                         listToFM [ (v+off, x')
