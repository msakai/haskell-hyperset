...
-- setToList :: Ord a => Set a -> [Either a (Set a)]
...
elementOf1 e (Set (g2,v2) t2) = any f (g2!v2)
    where f child = null (g2!child) && lookupFM t2 child == Just e -- XXX

elementOf2 :: (Ord u) => Set u -> Set u -> Bool
elementOf2 (Set (g1,v1) t1) (Set (g2,v2) t2) = any f (g2!v2)
    where f child = foo == take len (rt2!child)
...
construct g t = array (bounds g) [(v, wrap g' t' (m!v)) | v <- indices g]
    where ((g',t'), m) = mkCanonicalPicture (g,t)

wrap :: (Ord u) => Graph -> Tagging u -> Vertex -> Either u (Set u)
wrap g t n = case g!n of
